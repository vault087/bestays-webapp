...
import { IsPrivateToggleButton } from "@cms/modules/properties/form";
...
describe("PropertyPrivateToggleButton", () => {
  test("edits really edit the property", () => {
    const mockProperty = createMockProperty({ is_private: false });
    const { store } = renderWithProviders(<IsPrivateToggleButton />, { customProperty: mockProperty });

    const button = screen.getByRole("button");
    fireEvent.click(button);

    expect(store.getState().properties[mockProperty.id].is_private).toBe(true);
  });
  ...
  test("displayed value reflects changes", () => {
    ...
    expect(store.getState().properties[mockProperty.id].is_private).toBe(false);
